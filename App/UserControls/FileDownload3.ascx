...
<div id="dialog-form<%=ucID + '_' + ucAgency + '_' + ucVersion.Replace( '.', '_' )%>" class="popup_block">
    ...
    <table class="tbNoBorder" cellpadding="6">
        ...
        <tr>
            ...
            <td>
                <asp:TextBox ID="txtSeparator" Text="" MaxLength="1" Width="15px" runat="server" />
            </td>
        </tr>
        <tr>
            <td>
                <asp:Label ID="lblDelimiter" runat="server" Text="<%$ Resources:Messages,lbl_text_delimiter%>"></asp:Label>
            </td>
            <td>
                <asp:TextBox ID="txtDelimiter" Text="" MaxLength="1" Width="15px" runat="server" />
            </td>
        </tr>
    </table>
    <hr style="width: 100%" />
...
<script>
    $(document).ready(function () {
        ...
        $("#<%=lblDelimiter.ClientID %>, #<%=txtDelimiter.ClientID %>").hide();

            $("#<%=cmbDownloadType.ClientID %>").change(function () {
                var selectedItem = $(this).val();
                if (selectedItem == "CSV") {
                    $("#<%=lblSeparator.ClientID %>, #<%=txtSeparator.ClientID %>").show();
                    $("#<%=lblDelimiter.ClientID %>, #<%=txtDelimiter.ClientID %>").show();
                }
                else {
                    $("#<%=lblSeparator.ClientID %>, #<%=txtSeparator.ClientID %>").hide();
                    $("#<%=lblDelimiter.ClientID %>, #<%=txtDelimiter.ClientID %>").hide();
